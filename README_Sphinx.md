# Setup
- Go to a conda env `source activate tf2`.
- Install the module `cd here; pip install -e .`.
- Run sphinx script `bash mysphinx.sh bhishan`.

# Note
If we have already installed sphinx using now defunct conda environment, then we need to remove old sphinx binaries and reinstall again.
```bash
rm -rf ~/.local/bin/sphinx*
source activate tf2
