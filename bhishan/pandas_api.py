...
#============================= My extension ================================
    def describe(self, cols=None,select=None,style=True,print_=False,
        ...
        df: pandas.DataFrame
            Input dataframe.
        cols: list
            list of feature names of input dataframe
        select: list
            list of feature names of output description
            NOTE: If we only want to aggregate the built-in functions
            we can use .agg instead of slower .bp.describe(select)
            eg. df.assign(log_price=np.log1p(df['price']))[['price','log_price]]
        ...
        if style:
            ...
            # not transpose
                ...
                if select is None:
                    ...
                    subset = ['MissingPct','ZerosPct','OnesPct']
                    cond = set(subset).issubset(set(select))
                    if cond:
                        df_desc_styled = (df_desc.style
                            .apply(lambda x: ["background: salmon"
                                    if  str(v) in ['object','category']
                            ...
                            .background_gradient(subset=subset)
                            .format(fmt_dict,na_rep='')
                            )
                    else:
                        df_desc_styled = (df_desc[select].style
                            .apply(lambda x: ["background: salmon"
                                    if  str(v) in ['object','category']
                                    else ""
                                    for v in x], axis = 1)
    ...
    def value_counts(self, col,min_cnt=1,nrows=20):
        """Get nice table of value counts across multiple columns.
        ...
        col: str
            Name of column
        min_cnt: int
            Mininum value counts.
        nrows: int
            Number of rows in output dataframe.
        ...
        vc = vc[vc>=min_cnt]
        vcs = []
        for i in range(0,len(vc),nrows):
            cur_df = vc.iloc[i:i+nrows].reset_index()
...
#============== Plotting Numerical and Categorical Features =======
    def plot_num(self,num,xlim=None,figsize=(18,12),fontsize=24,xticks=None,
                ms=None,line_kws={},bins='auto',
                odir='images',ofile=None,save=True,show=False,
                print_=False,disp=False,dpi=300
                ):
    ...
    def plot_cat(self,cat,figsize=(12,8),fontsize=14,odir='images',ms=None,title=None,
    ...
    def plot_num_num(self,num1,num2,figsize=(12,8),fontsize=18,ms=None,
                    ...
                    odir='images',line_kws={},
                    ofile=None,save=True,show=False,dpi=300):
    ...
    def plot_cat_cat2(self,cat,target_cat,
        figsize=(12,8),ylim2=None,ms=None,odir='images',
        ofile=None,save=True,show=False,dpi=300):
...
#======================= Timeseries Analysis ==================
    def plot_date_cat(self,col_date,target_cat,figsize=(8,6),ms=None,
                    odir='images',
                    ofile=None,save=True,show=False,dpi=300):
    ...
    def plot_daily_cat(self,col_date,target_cat,figsize=(12,8),
            show_xticks=True,ms=None,odir='images',
            ofile=None,save=True,show=False,dpi=300):
...
#==================== Statistics ==================================
    def plot_corr(self,cols=None,target=None,topN=10,cmap='RdYlGn',
        ...
        odir='images',ofile=None,save=True,show=False,dpi=300):
        """Correlation plot.
    ...
    def plot_corr_sns(self,cols=None,target=None,topN=10,fontsize=None,
                    xrot=90,yrot=0,ms=None,odir='images',
                    ofile=None,save=True,show=True,dpi=300):
    ...
    def plot_statistics(self,cols=None,statistic='mean',color='b',
                        figsize=(12,4),ylim=None,
                        decimals=4, rot=30,percent=False,comma=False,dpi=300
                    ):
        ...
        if not os.path.exists('images'):
            os.makedirs('images')
        plt.savefig(f'images/{statistic}.png',dpi=dpi)
        plt.show()
        plt.close()

    def plot_ecdf(self,col,cross=None,bins=None,color='b',
                figsize=(12,4),xlim=None,ylim=None,fontsize=12,dpi=300):
        ...
        if not os.path.exists('images'):
            os.makedirs('images')
        plt.savefig(f'images/{col}_ecdf.png',dpi=dpi)
        plt.show()
        plt.close()

    def outliers_tukey(self,num,thr=1.5,plot_=True,ms=None,
            odir='images', ofile=None,save=True,show=False,dpi=300):
        """Get outliers based on Tukeys Inter Quartile Range.
    ...
    def compare_kde_binn(self,cols,binn,m=1,n=1,bw_adjust=0.5,
                    figsize=(12,8),fontsize=14,loc='best',ms=None,
                    odir='images',
                    ofile=None,save=True,show=False,dpi=300):
    ...
    def compare_kde2(self,num,binn,figsize=(12,8),fontsize=14,ms=None,
                    odir='images',
                    ofile=None,save=True,show=False,dpi=300):
        """Compare the KDE plots of two numerical features against binary target.
...
#============ Miscellaneous Plots ===============================
    def countplot(self,cols,m,n,
                    figsize=(12,8),fontsize=14,rot=45,ms=None,
                    odir='images',
                    ofile=None,save=True,show=False,dpi=300):
    ...
    def regplot_binn(self,cols1,cols2,binn,m,n,
            figsize=(12,8),fontsize=18,debug=False,ms=None,
            odir='images',ofile=None,save=True,show=False,dpi=300):
    ...
    def plot_boxplot_cats_num(self,cats,num,figsize=(12,8),ms=None,
                odir='images',
                ofile=None,save=True,show=False,dpi=300):
    ...
    def plot_count_cat(self, cat,
                percent=True,bottom=0,
                figsize=(12,8),fontsizex=18, fontsizey=18,
                horizontal=False,number=False,ms=None,
                odir='images',
                ofile=None,save=True,show=False,dpi=300):
